...
kubectl --kubeconfig ~/kubeconfigs/admin.kubeconfig apply -f coredns.yaml
```
...
## Verification
```bash
kubectl --kubeconfig ~/kubeconfigs/admin.kubeconfig -n kube-system get deploy coredns
kubectl --kubeconfig ~/kubeconfigs/admin.kubeconfig -n kube-system get pods -l k8s-app=kube-dns -w
```
Wait for READY 1/1 (or 2/2 if dual containers).

## Basic DNS Test
```bash
kubectl --kubeconfig ~/kubeconfigs/admin.kubeconfig run dns-test --image=busybox:1.36 --restart=Never -- sleep 3600
kubectl --kubeconfig ~/kubeconfigs/admin.kubeconfig exec dns-test -- nslookup kubernetes.default
```
Should resolve to 10.96.0.1.

Cleanup (optional):
```bash
kubectl --kubeconfig ~/kubeconfigs/admin.kubeconfig delete pod dns-test
```
