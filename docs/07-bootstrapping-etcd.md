...
The commands in this lab must be run on each master node: `master-1` and `master-2`. Log in to each of these using SSH.
...
```bash
{
  ...
  sudo mv etcd-v3.5.13-linux-amd64/etcd* /usr/local/bin/
}
```
...
The instance's internal IP address will be used to serve client requests and communicate with etcd cluster peers. Retrieve the internal IP address:

```bash
INTERNAL_IP=$(ip addr show enp0s8 | grep "inet " | awk '{print $2}' | cut -d / -f 1)
```

Each etcd member must have a unique name within an etcd cluster. Set the etcd name to match the hostname:
...
```
3a5796a6803f252, started, master-1, https://192.168.5.11:2380, https://192.168.5.11:2379
9bf9ccad8d8900a, started, master-2, https://192.168.5.12:2380, https://192.168.5.12:2379
```
