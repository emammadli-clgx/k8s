...
# Execute command silently and return output only - NEW FUNCTION
exec_on_node_silent() {
    local node=$1
    shift
    local cmd="$@"
...
# Copy file to remote node with error handling - FIXED VERSION
copy_to_node() {
    local node=$1
    local src=$2
    local dest=$3
...
# Configure Docker for Kubernetes with better error handling
configure_docker() {
    local node=$1
    
    log_info "Configuring Docker for Kubernetes on $node"
    
    # Backup existing Docker configuration
    exec_on_node "$node" "cp /etc/docker/daemon.json /etc/docker/daemon.json.backup 2>/dev/null || true"
    
    # Get Docker info to check current configuration
    docker_info=""
    if [[ "$node" == "master-1" ]] || [[ "$node" == "$(hostname)" ]]; then
        docker_info=$(sudo docker info 2>/dev/null || echo "")
    else
        docker_info=$(ssh -o StrictHostKeyChecking=no vagrant@$node "sudo docker info 2>/dev/null || echo ''")
    fi
    
    # Check if systemd is already configured
    if echo "$docker_info" | grep -q "Cgroup Driver: systemd"; then
        log_success "Docker is already configured with systemd cgroup driver on $node"
        return 0
    fi
    
    # Create Docker configuration directory
    exec_on_node "$node" "mkdir -p /etc/docker"
    
    # Create a safe Docker daemon configuration
    docker_config='{
  ...
  "storage-driver": "overlay2"
    ...
    if [[ "$node" == "master-1" ]] || [[ "$node" == "$(hostname)" ]]; then
        ...
        ssh -o StrictHostKeyChecking=no vagrant@$node "echo '$docker_config' | sudo tee /etc/docker/daemon.json > /dev/null"
    fi
    
    # Validate the JSON configuration
    if ! exec_on_node "$node" "python3 -m json.tool /etc/docker/daemon.json >/dev/null 2>&1"; then
        log_error "Docker daemon.json configuration is invalid on $node"
        # Restore backup if exists
        exec_on_node "$node" "cp /etc/docker/daemon.json.backup /etc/docker/daemon.json 2>/dev/null || rm -f /etc/docker/daemon.json"
        return 1
    fi
    ...
    # Try to restart Docker with better error handling
    log_info "Restarting Docker service on $node"
    if ! exec_on_node "$node" "systemctl restart docker"; then
        log_error "Failed to restart Docker on $node"
        
        # Show Docker service status for debugging
        log_info "Docker service status on $node:"
        exec_on_node "$node" "systemctl status docker --no-pager -l" || true
        
        # Show Docker logs
        log_info "Docker service logs on $node:"
        exec_on_node "$node" "journalctl -xeu docker.service --no-pager -l --since '5 minutes ago'" || true
        
        # Try to restore backup configuration
        log_info "Attempting to restore original Docker configuration on $node"
        exec_on_node "$node" "cp /etc/docker/daemon.json.backup /etc/docker/daemon.json 2>/dev/null || rm -f /etc/docker/daemon.json"
        exec_on_node "$node" "systemctl daemon-reload"
        
        if exec_on_node "$node" "systemctl restart docker"; then
            log_warning "Docker restored to original configuration on $node"
            log_warning "Continuing with original Docker configuration - may affect Kubernetes functionality"
        else
            log_error "Failed to restore Docker on $node"
            return 1
        fi
    else
        log_success "Docker restarted successfully with new configuration on $node"
    fi
    
    # Enable Docker service
    exec_on_node "$node" "systemctl enable docker"
    
    # Verify Docker is running
    sleep 5
    if service_running "$node" "docker"; then
        log_success "Docker is running on $node"
        ...
        if echo "$docker_info" | grep -q "Cgroup Driver: systemd"; then
            log_success "Docker is using systemd cgroup driver on $node"
        else
            log_warning "Docker may not be using systemd cgroup driver on $node"
        fi
    else
        log_error "Docker is not running on $node"
        return 1
    fi
}
...
install_container_runtime() {
    ...
    log_info "Checking container runtime on $node"
    
    # Check if Docker is already installed and running
    if command_exists "$node" "docker" && service_running "$node" "docker"; then
        log_success "Docker is already installed and running on $node"
        configure_docker "$node"
        return 0
    fi
    ...
    # If neither is installed, install containerd
...
for node in "${!NODES[@]}"; do
    ...
    exec_on_node "$node" "cat > /etc/modules-load.d/k8s.conf <<EOF
overlay
br_netfilter
EOF"
    ...
    exec_on_node "$node" "cat > /etc/sysctl.d/k8s.conf <<EOF
...
for node in "${!NODES[@]}"; do
    ...
    # Configure kubelet for low resource environments and Docker if present
    exec_on_node "$node" "mkdir -p /etc/systemd/system/kubelet.service.d"
    
    # Determine the container runtime
    runtime_args=""
    if command_exists "$node" "docker" && service_running "$node" "docker"; then
        runtime_args="--container-runtime=docker"
    else
        runtime_args="--container-runtime=remote --container-runtime-endpoint=unix:///var/run/containerd/containerd.sock"
    fi
    
    exec_on_node "$node" "cat > /etc/systemd/system/kubelet.service.d/20-low-resource.conf <<EOF
[Service]
Environment=\"KUBELET_EXTRA_ARGS=--feature-gates=NodeSwap=false --fail-swap-on=false $runtime_args\"
EOF"
    ...
    for cmd in kubeadm kubelet kubectl; do
        if command_exists "$node" "$cmd"; then
            version=""
            if [[ "$node" == "master-1" ]] || [[ "$node" == "$(hostname)" ]]; then
                version=$(sudo $cmd version --client --short 2>/dev/null | cut -d' ' -f3 || echo "unknown")
            else
                version=$(ssh -o StrictHostKeyChecking=no vagrant@$node "sudo $cmd version --client --short 2>/dev/null | cut -d' ' -f3" || echo "unknown")
            fi
...
# Create HAProxy configuration file directly on the remote node
exec_on_node "lb" "cat > /etc/haproxy/haproxy.cfg <<'EOF'
global
    log /dev/log local0
...
# Determine container runtime socket
cri_socket="unix:///var/run/containerd/containerd.sock"
if command_exists "master-1" "docker" && service_running "master-1" "docker"; then
    cri_socket="unix:///var/run/cri-dockerd.sock"
fi

# Create kubeadm configuration directly on master-1
exec_on_node "master-1" "cat > /tmp/kubeadm-config.yaml <<'EOF'
...
# Get join commands using silent execution - FIXED
...
# Determine container runtime socket for master-2
cri_socket_master2="unix:///var/run/containerd/containerd.sock"
if command_exists "master-2" "docker" && service_running "master-2" "docker"; then
    cri_socket_master2="unix:///var/run/cri-dockerd.sock"
fi

log_info "Joining master-2 to the cluster"
log_info "Join command: $CONTROL_PLANE_JOIN_CMD --cri-socket ${cri_socket_master2}"
exec_on_node "master-2" "$CONTROL_PLANE_JOIN_CMD --cri-socket ${cri_socket_master2}"
...
# Create Calico installation directly on master-1
exec_on_node "master-1" "cat > /tmp/calico-custom-resources.yaml <<'EOF'
...
cri_socket_worker1="unix:///var/run/containerd/containerd.sock"
if command_exists "worker-1" "docker" && service_running "worker-1" "docker"; then
    cri_socket_worker1="unix:///var/run/cri-dockerd.sock"
fi
exec_on_node "worker-1" "$WORKER_JOIN_CMD --cri-socket ${cri_socket_worker1}"

# Join worker-2
log_info "Joining worker-2 to the cluster"
cri_socket_worker2="unix:///var/run/containerd/containerd.sock"
if command_exists "worker-2" "docker" && service_running "worker-2" "docker"; then
    cri_socket_worker2="unix:///var/run/cri-dockerd.sock"
fi
exec_on_node "worker-2" "$WORKER_JOIN_CMD --cri-socket ${cri_socket_worker2}"
...
exec_as_vagrant "master-1" "cat <<EOF | kubectl apply -f -
...
# Create test deployment directly on master-1
exec_on_node "master-1" "cat > /tmp/nginx-test.yaml <<'EOF'
apiVersion: apps/v1
kind: Deployment
metadata:
  name: nginx-test
...
• Container Runtime: $(command_exists "master-1" "docker" && echo "Docker" || echo "containerd")
...
✓ Container Runtime (Docker/containerd with systemd cgroup driver)
